...
kubectl patch secret -n tkg-system "$TKG_MGMT_CLUSTER_NAME-pinniped-package" --type='json' -p='[{"op" : "replace" ,"path" : "/data/values.yaml" ,"value" : '$B64_NEW_CONFIG'}]'
...
if [ "$(kubectl get ns tanzu-system-auth --ignore-not-found)" ]
then
...
tanzu package installed kick $TKG_MGMT_CLUSTER_NAME-pinniped -n tkg-system -y --wait
